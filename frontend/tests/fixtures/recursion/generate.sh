#!/usr/bin/env bash
# Regenerates the recursion fixtures from the real backend. Never hand-edit
# the .json files — rerun this after tracer changes instead.
# Requires the backend on :8000 with the tracer Docker image built.
set -euo pipefail
cd "$(dirname "$0")"
for name in fib subsets nqueens mutual graph-dfs; do
  echo "tracing $name..."
  python3 - "$name" <<'EOF'
