#!/usr/bin/env bash
# Regenerates the graph fixtures from the real backend. Never hand-edit the
# .json files — rerun this after tracer changes instead.
# Requires the backend on :8000 with the tracer Docker image built.
set -euo pipefail
cd "$(dirname "$0")"
for name in dijkstra shortestPathDAG heap tree-traversal tree-levelorder tree-same tree-skewed; do
  echo "tracing $name..."
  python3 - "$name" <<'EOF'
